...
public class SunManager : MonoBehaviour
{
    ...
    void Update()
    {
		transform.localScale = new Vector3(starMass*10, starMass * 10, starMass * 10);
	...
	Color TemperatureToColor(float temperature)
	{
		...
		reColor.r = Mathf.Max(Mathf.Min((56100000f * Mathf.Pow(temperature, -3f / 2f) + 148f) / 255f, 1.0f), 0.0f);
		reColor.g = Mathf.Max(Mathf.Min((100.04f * Mathf.Log(temperature) - 623.6f) / 255f, 1.0f), 0.0f);
		reColor.b = Mathf.Max(Mathf.Min((194.18f * Mathf.Log(temperature) - 1448.6f) / 255f, 1.0f), 0.0f);
		if (temperature > 6500)
		{
			reColor.g = Mathf.Max(Mathf.Min((35200000 * Mathf.Pow(temperature, -3f / 2f) + 148f) / 255f, 1.0f), 0.0f);
		}
		return reColor;
	}
}
