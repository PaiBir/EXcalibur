	...
	void Update()
	{
		if (UpdateMesh)
		{
			...
			for (int i= 0; i < Extriangles.Length; i++)
			{
				...
				//Check for poles
				/*if (Mathf.Abs(v2.x) > 3.1)
				{
					v2.x = (rootVertex.x + v3.x) / 2;
				} else if (Mathf.Abs(v3.x) > 3.1)
				{
					v3.x = (rootVertex.x + v2.x) / 2;
				} else if (Mathf.Abs(rootVertex.x) > 3.1)
				{
					rootVertex.x = (v2.x + v3.x) / 2;
				}*/

				Debug.Log("Root: " + rootVertex + ", V2: " + v2 + ", V3: " + v3);
				...
				/*if (dir1.y > Mathf.PI / (subdivisions + 1f))
				{
					dir1.y -= (2 * Mathf.PI) / (subdivisions + 1f);
				}*/

				/*if (dir2.y < -Mathf.PI / (subdivisions + 1f))
				{
					dir2.y += 2* Mathf.PI;
				}*/
				...
				for (int j = 0; j <= subdivisions; j++)
				{
					...
					for (int k = 0; k < thisGen.Length; k++)
					{
						...
						Vector2 NRoot = V2mod(thisGen[k], new Vector2(Mathf.PI * 2, Mathf.PI), new Vector2(0, 1));
						Vector2 NDir1 = V2mod(thisGen[k] + dir1, new Vector2(Mathf.PI * 2, Mathf.PI), new Vector2(0, 1));
						Vector2 NDir2 = V2mod(thisGen[k] + dir2, new Vector2(Mathf.PI * 2, Mathf.PI), new Vector2(0, 1));
						...
						if (IsRootIn)
						{
							Debug.Log("Root vertex  " + NRoot + " already exists " + activeVerts[Rindex] + ". Index: " + Rindex);
							activeFaces[numFaces * 3] = Rindex;
						}
						else
						{
							activeVerts[fvC] = NRoot;
							Debug.Log("Root vertex " + NRoot + " does not exist. Creating new index: " + fvC);
						...
						if (IsDir1In)
						{
							Debug.Log("Dir1 vertex " + NDir1 + " already exists " + activeVerts[D1index] + ". Index: " + D1index);
							activeFaces[(numFaces * 3) + 1] = D1index;
						}
						else
						{
							activeVerts[fvC] = NDir1;
							Debug.Log("Dir1 vertex " + NDir1 + " does not exist. Creating new index: " + fvC);
						...
						if (IsDir2In)
						{
							Debug.Log("Dir2 vertex " + NDir2 + " already exists " + activeVerts[D2index] + ". Index: " + D2index);
						...
						else
						{
							...
							Debug.Log("Dir2 vertex " + NDir2 + " does not exist. Creating new index: " + fvC);
			...
			Vector3[] meshVerts = converter.ConvertSphericaltoCartesian(activeVerts);
			Vector3[] meshNorms = new Vector3[meshVerts.Length];

			//Calculate Normals
			Debug.Log("Generating Normals!");
			for (int i = 0; i < meshNorms.Length; i++)
			{
				meshNorms[i] = meshVerts[i].normalized;
			}

			//Calculate Colors
			Debug.Log("Generating Colors!");
			Color[] vertcols = new Color[meshVerts.Length];
			...
			seeingEye3 = activeFaces.ToArray();
